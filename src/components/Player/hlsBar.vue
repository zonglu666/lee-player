      ...
      <div class="process" @click="changePlayedTime($event)">
        ...
        <div
          ...
          :key="m.id"
            ...
            :style="`display: ${isLive ? 'none' : 'block'}; left: calc(${
              options.url ? ((m.time >= fullTime ? fullTime : m.time) * 100) / fullTime : 0
            }% - 3px); background: ${hasPlayedTime > m.time ? '#fff' : '#10c8af'}`"
          >
      ...
      _this.$nextTick(() => {
        document.onkeydown = function (e){
          ...
          if (e.keyCode === 38 || e.key === 'ArrowUp') {
            ...
            _this.$emit('changeOptions', {type: 'volume', value: _this.volume});
            e.stopPropagation()
            _this.keyCodeVol();
            return false;
          }

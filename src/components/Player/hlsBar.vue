...
export default {
  ...
  watch: {
    ...
    cssScreen(n, o) {
      if (n !== o) {
        this.$emit('fullScreen', { type: 'cssFullScreen', status: n });
      }
    },
    fullScreen(n, o) {
      if (n !== o) {
        this.$emit('fullScreen', { type: 'fullScreen', status: n });
      }
    },
  },
  mounted() {
  ...
  methods: {
      ...
      this.countTime = setTimeout(() => {
        if (this.hasPlayedTime >= this.fullTime) {
          if (this.isLive) {
            this.fullTime =
              this.fullTime +
              this.vplayer.bufferController.details.totalduration;
        ...
        this.hasPlayedPoint =
          this.hasPlayedTime > this.hasPlayedPoint
            ? this.hasPlayedTime
            : this.hasPlayedPoint;
      }, 1000);
    },
    ...
    changePlayedTime(ev) {
      ...
      if (process) {
        ...
        this.vplayer
          ? (this.vplayer._media.currentTime = this.hasPlayedTime)
          : null; // 原生dom指定播放位置
      }
    },
    // 向上传递 当前播放时长
    ...
    toggleVolumeStatus() {
      ...
      this.vplayer
        ? (this.vplayer._media.volume = this.openVolume ? this.volume / 100 : 0)
        : null; // 原生dom指定音量
      this.$emit('changeOptions', { type: 'volume', value: this.volume });
    },
    // 开始触发播放位置
    ...
    initPlayer() {
      ...
      if (Hls.isSupported()) {
        ...
        this.vplayer._media.volume = this.volume / 100;
        this.vplayer._media.playbackRate = this.speedType;
        // 完成 video 节点绑定
        this.vplayer.on(Hls.Events.MEDIA_ATTACHED, (e, data) => {
          ...
          this.vplayer.on(Hls.Events.MANIFEST_LOADED, (e, mdata) => {
            ...
            // 直播，获取缓冲区
            this.isLive &&
              this.vplayer.on(Hls.Events.LEVEL_LOADED, (e, ldata) => {
            ...
            // 点播，获取缓冲区
            !this.isLive &&
              this.vplayer.on(Hls.Events.FRAG_BUFFERED, (e, fdata) => {
    ...
    startPlay(video) {
      let timm = setTimeout(() => {
        this.hasPlayedTime = video ? video.currentTime : 0;
        // this.preloadTime = video && video.buffered.length ? video.buffered.end(0) : this.preloadTime;
        const delay = this.preloadTime - this.hasPlayedTime;
        if (
          this.vplayer &&
          (this.preloadTime > this.hasPlayedTime || !this.isLive)
        ) {
          ...
          if (this.options.autoplay) {
            this.playnum++;
            this.playnum < 2 ? this.playOrPause() : null;
          }
          if (this.vplayer._media.paused) {
            ...
            if (delay < 1 && this.hasPlayedTime < this.fullTime) {
              ...
              this.vplayer._media.currentTime = this.preloadTime;
              this.vplayer._media.play();
            } else {
            ...
            video.addEventListener('error', () => {
              if (video && video.error) {
                ...
                this.loading = true;
                this.urlStatus = false;
                this.urlStatusText = video.error.message;
              }
            });
          }
          clearTimeout(timm);
      ...
      _this.$nextTick(() => {
        document.onkeydown = function (e) {
          ...
          if (e.keyCode === 38 || e.key === 'ArrowUp') {
            ...
            _this.vplayer
              ? (_this.vplayer._media.volume = _this.volume / 100)
              : null;
          ...
          if (e.keyCode === 37 || e.key === 'ArrowLeft') {
            ...
            _this.vplayer
              ? (_this.vplayer._media.currentTime = _this.hasPlayedTime)
              : null;
    ...
    changeSubPlayer(e, sub) {
      ...
      this.vplayer._media.currentTime = sub.time;

  ...
  methods: {
    ...
    pipChangePlayedTime(ev) {
      ...
      if (process) {
        ...
        this.pipPlayer
          ? (this.pipPlayer.currentTime = this.pipHasPlayedTime)
          : null; // 原生dom指定播放位置
      }
    },
    // 点击 更改音量
    ...
    pipToggleVolumeStatus() {
      ...
      this.pipPlayer
        ? (this.pipPlayer.volume = this.pipOpenVolume
            ? this.pipVolume / 100
            : 0)
        : null; // 原生dom指定音量
    },
    ...
    pipInitPlayer() {
      ...
      this.pipPlayer.addEventListener('loadedmetadata', () => {
        //加载数据
        ...
        this.pipPlayer.addEventListener('timeupdate', () => {
          ...
          this.pipPreloadTime =
            this.pipPlayer && this.pipPlayer.buffered.length
              ? this.pipPlayer.buffered.end(0)
              : this.pipPreloadTime;
          this.pipPreloadTime =
            this.pipPreloadTime > this.pipFullTime
              ? this.pipFullTime
              : this.pipFullTime - this.pipPreloadTime < 1
              ? this.pipFullTime
              : this.pipPreloadTime;
    ...
    pipKeyCodeVol() {
      ...
      vk.className = vk.className.includes('pip-volume-keycode-show')
        ? vk.className
        : vk.className + ' pip-volume-keycode-show';
      this.pipKeyVolInterval = setInterval(() => {
        tm--;
        if (tm <= 0) {
          vk.className = vk.className.includes('pip-volume-keycode-show')
            ? vk.className.split(' pip-volume-keycode-show')[0]
            : vk.className;
      ...
      _this.$nextTick(() => {
        ...
        point.onmousedown = function (event) {
          ...
          document.onmousemove = function (ev) {
            ...
            _this.pipHasPlayedTime = nowTime;
            _this.pipPlayer
              ? (_this.pipPlayer.currentTime = _this.pipHasPlayedTime)
              : null;
            _this.pipDragVolumeLen = parseInt(
              Math.abs(pipDragVolumeLen - _this.pipHasPlayedTime)
            );

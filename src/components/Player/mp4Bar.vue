...
export default {
  ...
  methods: {
    ...
    toggleVolumeStatus(){
      ...
      this.vplayer ? this.vplayer.volume = this.openVolume ? this.volume / 100 : 0 : null; // 原生dom指定音量
      this.$emit('changeOptions', {type: 'volume', value: this.volume});
    },
    // 初始化 MP4 播放器
    createPlayer() {
    ...
    initPlayer() {
      ...
      if(this.vplayer){
        ...
        this.vplayer.addEventListener('loadedmetadata', () => { //加载数据
            this.loading = false;
            //视频的总长度
            ...
            this.vplayer.addEventListener('timeupdate', () => {
              ...
              this.preloadTime = this.vplayer && this.vplayer.buffered.length ? this.vplayer.buffered.end(0) : this.preloadTime;
              this.preloadTime = this.preloadTime > this.fullTime ? this.fullTime : this.fullTime - this.preloadTime < 1 ? this.fullTime : this.preloadTime;
      ...
      _this.$nextTick(() => {
        document.onkeydown = function (e){
          ...
          if (e.keyCode === 37 || e.key === 'ArrowLeft') {
            ...
            _this.vplayer ? _this.vplayer.currentTime = _this.hasPlayedTime : null;

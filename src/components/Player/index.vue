  ...
  >
    ...
    <div v-if="subType" class="sub-position" @mousedown="subMouseEvent">
      <pipMp4
        v-if="subType === 'mp4'"
        :options="subData"
        :subPipStatus="subPipStatus"
        @destroyPipPlayer="destroyPipPlayer"
      ></pipMp4>
      <pipHls
        v-if="subType === 'hls'"
        :options="subData"
  ...
  methods: {
    ...
    initSubPlayer(sub) {
      ...
      this.subData = {
        ...
        lang: 'zh', // 语言：en | zh
        theme: '#000', // 主题色
        url: [{ quality: 'high', url: sub.url }], // 视频链接
        subUrl: [],
    ...
    subMouseEvent(event) {
      ...
      document.onmousemove = function (ev) {
        ...
        sub.style.left =
          (subleft + sub.offsetWidth > processW
            ? processW - sub.offsetWidth
            : subleft) + 'px';
        sub.style.top =
          (subtop + sub.offsetHeight > processH
            ? processH - sub.offsetHeight
            : subtop) + 'px';

...
export default {
  ...
  methods: {
    ...
    pipPlayOrPause() {
      ...
      this.pipPlayStatus && this.subPlayer
        ? this.subPlayer._media.play()
    ...
    pipInitPlayer() {
      ...
      if (Hls.isSupported()) {
        ...
        this.subPlayer._media.volume = this.pipVolume / 100;
        this.subPlayer._media.playbackRate = this.pipSpeedType;
        // 完成 video 节点绑定
        this.subPlayer.on(Hls.Events.MEDIA_ATTACHED, (e, data) => {
          // console.log("完成 video 节点绑定", Hls.Events, e, data);
          this.subPlayer.loadSource(source);
          // [m3u8链接、*.ts链接]成功
          this.subPlayer.on(Hls.Events.MANIFEST_LOADED, (e, mdata) => {
        ...
        if(this.subPlayer && (this.pipPreloadTime > this.pipHasPlayedTime || !this.pipIsLive)){
          ...
          if(this.subPlayer._media.paused){
            ...
            if(delay < 1 && this.pipHasPlayedTime < this.pipFullTime){
              ...
              this.subPlayer._media.currentTime = this.pipPreloadTime;
              this.subPlayer._media.play();
            } else {
